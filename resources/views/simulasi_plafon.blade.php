...
<head>
    ...
    <style>
        .glow-indigo { box-shadow: 0 10px 30px -10px rgba(99, 102, 241, 0.15); }
        .glow-yellow { box-shadow: 0 10px 30px -10px rgba(245, 158, 11, 0.15); }
        .glow-red { box-shadow: 0 10px 30px -10px rgba(239, 68, 68, 0.15); }
        .glass-card {
            background: #ffffff;
...
<body class="bg-white text-slate-800 font-sans min-h-screen flex items-center justify-center p-6 relative overflow-hidden">
    ...
    <div class="max-w-6xl w-full relative z-10">
        ...
        <div class="text-center mb-8">
            <span class="px-3 py-1 text-xs font-semibold bg-indigo-50 text-indigo-605 border border-indigo-100 rounded-full uppercase tracking-widest">
            ...
            <p class="text-slate-550 text-sm mt-1">Keputusan akhir plafon belanja setelah proses pembulatan, batas minimal, dan cek tunggakan</p>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-stretch">
            ...
            <div class="lg:col-span-5 flex flex-col justify-between glass-card rounded-3xl p-8 glow-indigo relative overflow-hidden" id="card-outer-glow">
                ...
                <!-- Equation Math Recap -->
                <div class="bg-slate-50 rounded-2xl p-4 border border-slate-200 mt-6 text-xs space-y-1.5">
                    <span class="text-slate-400 font-bold text-[10px] uppercase block mb-1">Perkalian Dasar:</span>
                    <div id="math-recap" class="font-mono text-slate-600">
            ...
            <div class="lg:col-span-7 glass-card rounded-3xl p-8 flex flex-col justify-between">
                <div>
                    <h2 class="text-xs font-bold text-indigo-600 uppercase tracking-wider mb-6">2. Aturan Plafon &amp; Input Simulator</h2>
                    
                    <!-- Simulating slider inputs -->
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mb-6">
                        <!-- Limit Base Input -->
                        <div>
                            <label class="block text-xs text-slate-500 mb-1">Simulasikan Limit Base (Rp)</label>
                            <input type="text" id="in-limit-base" value="3.600.000" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-sm text-slate-800 font-mono focus:outline-none focus:bg-white focus:border-indigo-500" oninput="formatNumberInput(this); processRules()">
                        </div>

                        <!-- TS Multiplier Selector -->
                        <div>
                            <label class="block text-xs text-slate-500 mb-1">Faktor TS (Multiplier)</label>
                            <select id="in-ts-factor" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-sm text-slate-800 focus:outline-none focus:bg-white focus:border-indigo-500" onchange="processRules()">
                        ...
                        <!-- Arrears Toggle -->
                        <div class="md:col-span-2 flex items-center justify-between p-4 rounded-xl bg-rose-50 border border-rose-100">
                            <div>
                                <span class="block text-xs font-semibold text-rose-600">Tunggakan Aktif (Active Arrears)</span>
                            ...
                            <label class="relative inline-flex items-center cursor-pointer">
                                ...
                                <div class="w-10 h-6 bg-slate-200 rounded-full peer peer-focus:outline-none dark:bg-slate-300 peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-slate-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-rose-600"></div>
                            </label>
                        </div>
                    </div>
                    ...
                    <div class="space-y-3.5">
                        <h3 class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Alur Validasi Ketentuan</h3>
                        
                        <!-- Checkpoint 1: Perhitungan -->
                        <div class="flex items-start gap-3 text-xs">
                            <div id="check-icon-calc" class="w-5 h-5 rounded-full bg-emerald-50 text-emerald-600 flex items-center justify-center border border-emerald-200 mt-0.5">
                                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 13l4 4L19 7"></path></svg>
                            </div>
                            <div>
                                <span class="font-semibold text-slate-700">1. Hasil Perkalian Faktor TS</span>
                                <p id="check-desc-calc" class="text-[10px] text-slate-400 mt-0.5">3.600.000 &times; 1.5 = 5.400.000</p>
                            </div>
                        </div>

                        <!-- Checkpoint 2: Pembulatan -->
                        <div class="flex items-start gap-3 text-xs">
                            <div id="check-icon-round" class="w-5 h-5 rounded-full bg-emerald-50 text-emerald-600 flex items-center justify-center border border-emerald-200 mt-0.5">
                                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 13l4 4L19 7"></path></svg>
                            </div>
                            <div>
                                <span class="font-semibold text-slate-700">2. Pembulatan ke Ribuan Terdekat</span>
                                <p id="check-desc-round" class="text-[10px] text-slate-400 mt-0.5">Dibulatkan ke ribuan terdekat</p>
                            </div>
                        </div>

                        <!-- Checkpoint 3: Minimum Limit -->
                        <div class="flex items-start gap-3 text-xs">
                            <div id="check-icon-min" class="w-5 h-5 rounded-full bg-emerald-50 text-emerald-600 flex items-center justify-center border border-emerald-200 mt-0.5">
                                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 13l4 4L19 7"></path></svg>
                            </div>
                            <div>
                                <span class="font-semibold text-slate-700">3. Validasi Batas Minimum Limit (Rp100.000)</span>
                                <p id="check-desc-min" class="text-[10px] text-slate-400 mt-0.5">Memenuhi batas minimum Rp100.000</p>
                            </div>
                        </div>

                        <!-- Checkpoint 4: Tunggakan Check -->
                        <div class="flex items-start gap-3 text-xs">
                            <div id="check-icon-arrears" class="w-5 h-5 rounded-full bg-emerald-50 text-emerald-600 flex items-center justify-center border border-emerald-200 mt-0.5">
                                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 13l4 4L19 7"></path></svg>
                            </div>
                            <div>
                                <span class="font-semibold text-slate-700">4. Pengecekan Tunggakan Aktif</span>
                                <p id="check-desc-arrears" class="text-[10px] text-slate-400 mt-0.5">Bebas tunggakan aktif</p>
                            </div>
                        </div>
                    </div>
                </div>

                <div class="mt-6 text-[10px] text-slate-400 italic text-center">
    ...
    <script>
        ...
        function processRules() {
            ...
            if(roundedLimit > 0 && roundedLimit < 100000) {
                ...
                minRuleApplied = true;
                document.getElementById('check-desc-min').innerHTML = `<span class="text-amber-600 font-semibold">Limit di bawah Rp100.000 &rarr; Dibulatkan naik ke Rp100.000</span>`;
                document.getElementById('check-icon-min').className = "w-5 h-5 rounded-full bg-amber-50 text-amber-600 flex items-center justify-center border border-amber-200 mt-0.5";
            } else {
                ...
                document.getElementById('check-icon-min').className = "w-5 h-5 rounded-full bg-emerald-50 text-emerald-600 flex items-center justify-center border border-emerald-200 mt-0.5";
            }
            ...
            if(hasArrears) {
                ...
                checkIconArr.className = "w-5 h-5 rounded-full bg-rose-50 text-rose-600 flex items-center justify-center border border-rose-200 mt-0.5";
                ...
                checkIconArr.className = "w-5 h-5 rounded-full bg-emerald-50 text-emerald-600 flex items-center justify-center border border-emerald-200 mt-0.5";
            ...
            if(hasArrears) {
                recapBox.innerHTML = `<span class="text-rose-600 font-bold uppercase tracking-wider">Arrears Override (Tunggakan Aktif) &rarr; Plafon Kredit: Rp0</span>`;
            } else if (tsFactor === 0) {
                recapBox.innerHTML = `<span class="text-rose-600 font-bold uppercase tracking-wider">TS &lt; 55 (Faktor 0.0x) &rarr; Plafon Kredit: Rp0</span>`;
            } else {
                let text = `${formatRupiah(limitBase)} × ${tsFactor.toFixed(1)} = ${formatRupiah(limitBase * tsFactor)}`;
                if(minRuleApplied) {
                    text += ` <span class="text-amber-600">(Dibulatkan ke Min. Rp100.000)</span>`;
                } else {
                    text += ` <span class="text-slate-400">(Dibulatkan ke ribuan terdekat)</span>`;
                }
